...
const HomeworkView: React.FC = () => {
  ...
  return (
    <div className="max-w-7xl mx-auto py-6 md:py-10 px-2 md:px-4 space-y-8 md:space-y-12">
      <div className="text-center space-y-4">
        <h1 className="text-4xl md:text-8xl font-black text-white tracking-tighter">{isAr ? 'الواجبات' : 'Homework'}</h1>
        <div className="flex items-center justify-center gap-3">
          <div className="h-px w-8 md:w-12 bg-blue-500/50"></div>
          <p className="text-blue-500 text-xs md:text-xl font-black uppercase tracking-[0.4em]">{isAr ? 'مساعد الحل بالذكاء الاصطناعي' : 'AI Vision Solver'}</p>
          <div className="h-px w-8 md:w-12 bg-blue-500/50"></div>
        </div>
      </div>

      <div className={`grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10 items-start ${isAr ? 'rtl' : 'ltr'}`}>
        <div className="lg:col-span-4 space-y-8 order-2 lg:order-1">
          <div className="bg-[#111827]/40 border border-white/5 p-6 md:p-8 rounded-[2.5rem] space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {subjects.map((s) => {
                ...
                return (
                  <button key={s.id} onClick={() => setSubject(s.id)} className={`flex flex-col items-center gap-3 p-4 md:p-5 rounded-2xl border transition-all ${isActive ? 'bg-blue-600/20 border-blue-500 shadow-xl text-blue-400' : 'bg-black/40 border-white/5 text-slate-500 hover:border-white/10'}`}>
        ...
        <div className="lg:col-span-8 space-y-8 order-1 lg:order-2">
          <div className="bg-[#111827]/60 border border-white/10 rounded-[2rem] md:rounded-[3rem] p-6 md:p-12 shadow-2xl relative">
            <div className="relative group">
              {selectedImage && (
                <div className="absolute top-0 right-0 z-20 animate-in zoom-in-95 duration-300 transform rotate-2">
                  <div className="relative">
                    <img src={selectedImage} alt="OCR Preview" className="w-32 h-32 md:w-48 md:h-48 object-contain rounded-3xl border-4 border-blue-600 shadow-2xl bg-black" />
              ...
              <textarea
                ...
                className={`w-full min-h-[250px] md:min-h-[350px] bg-black/40 border border-white/5 rounded-[1.5rem] md:rounded-[2rem] p-6 md:p-12 focus:outline-none focus:ring-4 focus:ring-blue-500/10 text-lg md:text-3xl leading-relaxed text-slate-100 placeholder:text-slate-800 resize-none transition-all ${isAr ? 'text-right' : 'text-left'}`}
                dir={isAr ? 'rtl' : 'ltr'}
              />
              {loading && (
            ...
            <div className={`flex flex-col sm:flex-row gap-5 mt-8 ${isAr ? 'flex-row-reverse' : ''}`}>
              ...
              <button onClick={() => fileInputRef.current?.click()} className="flex items-center gap-3 px-8 py-4 md:py-5 bg-black/60 border border-white/10 rounded-2xl font-black text-xs uppercase tracking-widest text-slate-500 hover:text-blue-400 transition-all justify-center">
                <ImageIcon className="w-5 h-5" />
                {isAr ? 'رفع صورة (OCR)' : 'Upload Photo (OCR)'}
              </button>
              <button onClick={handleSolve} disabled={loading || (!question.trim() && !selectedImage)} className="flex-1 bg-blue-600 hover:bg-blue-500 text-white font-black py-4 md:py-5 rounded-2xl flex items-center justify-center gap-4 transition-all shadow-2xl shadow-blue-600/30">
                <Sparkles className="w-5 h-5" />
                <span className="text-lg md:text-xl uppercase tracking-widest">{isAr ? 'حل الواجب' : 'Solve with Teacher AI'}</span>
              </button>
            </div>
          </div>

          {solution && (
            <div className="bg-[#111827]/40 border border-blue-500/20 rounded-[2rem] md:rounded-[3rem] p-8 md:p-16 animate-in fade-in slide-in-from-top-6 duration-1000 shadow-2xl">
              ...
              <div className={`prose prose-invert max-w-none text-slate-100 text-base md:text-2xl leading-relaxed ${isAr ? 'text-right' : 'text-left'}`} dir={isAr ? 'rtl' : 'ltr'}>
                {solution.split('\n').map((line, i) => (
